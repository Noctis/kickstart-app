...
There were three major changes regarding configuration classes between Kickstart 2.3.0 and 3.0.0:
...
### 4.1 If You Wish to Remove `FancyConfiguration`
...
`App\Configuration\FancyConfigurationInterface` and nothing else. If it looks like this:
...
remove references to it from the application's HTTP and CLI entry points, i.e.:

* `public/index.php`, and
* `bin/console` files

and finally remove the `ConfigurationProvider` service provider, the `src/Configuration/FancyConfigurationInterface.php` 
and the `src/Configuration/FancyConfiguration.php` files.

### 4.2 If You Wish to Keep `FancyConfiguration`
