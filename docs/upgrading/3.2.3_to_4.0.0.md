...
values from the request's body or query string. Any calls to the `get()` method in your actions will need to be replaced 
by either:

* `fromQueryString()`, for requests made _via_ `GET`, or
* `fromBody()`, for requests made _via_ `POST`.

The two new methods have the exact same signature as `get()` used to have.
